...
def _agg_from(subset: pd.DataFrame, top_n: int, ascending: bool = False):
    agg = (
        ...
        .reset_index()
        .sort_values(["avg_impact", "avg_rank"], ascending=[ascending, not ascending])
    )
...
def _build_response(benefit: str, subset: pd.DataFrame, top_n: int, category: Optional[str] = None,
    ...
    summary = (
        f"For '{benefit}'{scope}, the {which} driver is '{lead['attribute']}' with an average impact of "
        f"{round(lead['avg_impact'],1)} points (avg rank {round(lead['avg_rank'],2)} across "
...
def _compare(b1_text: str, b2_text: str, top_n: int) -> dict:
    ...
    summary = (
        ...
        f"({round(top1.iloc[0]['avg_impact'],1)} pts); top driver for '{b2}' is "
        f"'{top2.iloc[0]['attribute']}' ({round(top2.iloc[0]['avg_impact'],1)} pts)."
